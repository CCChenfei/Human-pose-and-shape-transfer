...
def batch_worlds_relate(rot_mat,name=None):

    with tf.name_scope(name,"batch_worlds_relate",[rot_mat]):
        batch_size=rot_mat.shape[0].value
        rotx=batch_rodrigues(tf.constant([[0.,0.,0.]]*batch_size))
        roty=batch_rodrigues(tf.constant([[0.,0.,0.]]*batch_size))
        result=tf.matmul(rotx,rot_mat)
        result=tf.matmul(roty,result)

        return result

def batch_inv_rodrigues(rot_mat,name=None):
    """
    ...
    :return:
    """
    with tf.name_scope(name,"batch_inv_rodrigues",[rot_mat]):
        theta=tf.abs(tf.acos(tf.clip_by_value((rot_mat[:,0,0]+rot_mat[:,1,1]+rot_mat[:,2,2]-1.)/2.,-1,1)))
        m=(theta+1e-8)/(2.*tf.sin(theta)+1e-8)
        ax=tf.stack([
        ...
        return tf.expand_dims(m,axis=-1)*ax

def batch_inv_rodrigues2(rot_mat,name=None):
    """
    ...
    with tf.name_scope(name,"batch_inv_rodrigues",[rot_mat]):
        ...
        return tf.expand_dims(m,axis=-1)*ax,theta,m,ax
