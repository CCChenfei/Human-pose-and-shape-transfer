...
def main(config):
    ...
    img = np.reshape(img, [config.batch_size * config.use_cam_batch, config.img_size,
                           config.img_size, 3])
    # bg = bg[:, 0, :, :, :]
    # bg = np.expand_dims(bg, axis=1)
    # bg = np.tile(bg, [1, config.use_cam_batch, 1, 1, 1])
    bg = np.reshape(bg, [config.batch_size * config.use_cam_batch, config.img_size,
                         config.img_size, 3])
    R = np.reshape(R, [config.batch_size * config.use_cam_batch, 3, 3])

    result=[]
    result = trainer.test(img,bg)
    # for i in range(config.useCamBatches):
    #     result.append(trainer.test(img[:,i],bg[:,i]))

    if config.output_path is not None:
        mkdir(config.output_path)
        for c in range(config.use_cam_batch):
            for i in range(config.batch_size):
                global_name=os.path.join(config.output_path,'res_i%d_c%d'%(i,c))

                cv2.imwrite( global_name+'_input.png', denormalize_rgb(img[4*i+c]))
                cv2.imwrite(global_name + '_bg.png', denormalize_rgb(bg[4*i+c]))
                ...
                name = os.path.join(config.output_path, 'res_i%d_c%d' % (i, c))
                cv2.imwrite(name + '_final.png', denormalize_rgb(result['img'][4*i+c]))
